  ...
  $.ajax({
    ...
    success: function (response) {     
      ...
      if (response && response.getitem) {
        ...
        // if (response.getitem.variants_json) {
        //   for (var i = 0; i < response.getitem.variants_json.length; i++) {
        //     html += '<p class="variant_name variant_name text-dark fw-500 pro-title line-2 mb-2 fs-6">' +
        //       response.getitem.variants_json[i].variant_name +
        //       "</p>";

        //     html += '<select name="product[' + [i] + ']" id="pro_variants_name" class="form-control variant-selection  pro_variants_name' +
        //       [i] + ' pro_variants_name variant_loop variant_val mb-2 py-1">';

        //     for (var t = 0; t < response.getitem.variants_json[i].variant_options.length; t++) {
        //       html += '<option value="' +
        //         response.getitem.variants_json[i].variant_options[t] +
        //         '" id="' +
        //         response.getitem.variants_json[i].variant_options[t] +
        ...
        if (response.getitem.variants_json) {
          for (var i = 0; i < response.getitem.variants_json.length; i++) {
            var variant = response.getitem.variants_json[i];
            
            // Add variant name
            html += '<p class="variant_name variants title ">' +
              variant.variant_name +
              '</p>';
            
            // Add container for radio buttons
            html += '<div class="variant-options-container">';
            
            for (var t = 0; t < variant.variant_options.length; t++) {
              var optionValue = variant.variant_options[t];
              // console.log(optionValue);
              
              var isChecked = (t === 0) ? ' checked' : ''; 
              html += '<div class="form-check mb-2">';
              html += '<input class="form-check-input variant-selection variant_loop variant_val" type="radio" ' +
                'name="product[' + i + ']" id="' + optionValue + '_variant_option" ' +
                'value="' + optionValue + '"' + isChecked + '>';
              html += '<label class="py-0 form-check-labels " for="' + optionValue + '_variant_option"><span class="modal-price">' +
                optionValue +
                '</span></label>';
              html += '</div>';
            }
            html += '</div>'; // Close variant-options-container
          }
        }
        // Update stock status
...
$(document)
  .on("change", ".variant-selection", function () {
    set_variant_price();
  })
  .change();
...
function set_variant_price() {
  var variants = [];
  $(".variant-selection:checked").each(function (index, element) {
    variants.push(element.value);
  });
...
// function placeorder(placeorderurl) {
//   // Get and clean amounts
//   var discount_amount = $('#orderdiscount_amount').text().trim(); 
//   var cleaned_amount = discount_amount.replace(/[^0-9.-]/g, '');
//   var numeric_amount = parseFloat(cleaned_amount);

//   var order_note = $('#cart_order_note').text().trim();
